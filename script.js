...
const TARGETHero = document.getElementById('TARGETHero');
...
window.addEventListener('scroll', function() {
    ...
    TARGETHero.style.transform = `translateX(${-(scrollY / 4)}px)`;
    ...
    if (aboutParaReveal < windowHeight - aboutParaRevealPoint) {
        if (!typed) {
            typed = true;
            textTypingEffect(about_p,"TARGET's mission is to empower startups and entrepreneurs by providing them with the resources, support, and expertise they need to turn their visions into reality.")
        }
    }else{
        typed = false;
        about_p.textContent = '';
...
//template_ji3m51r
//service_zb9wmag
//2f9HzfcBIOOXGej3UscOs
const sendEmail = (e)=>{
    e.preventDefault();

    emailjs.sendForm('service_zb9wmag','template_ji3m51r','#contactForm','IprlpMJ8BJcS60GOK')
    .then(()=>{
